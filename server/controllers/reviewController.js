...
export const addReview = async (req, resizeBy, next) => {
  try {
    ...
    console.error(error);
    next(ApiError.badRequest("Ошибка при добавлении комментариев"));
  }
};
...
export const getReviewsByOfferId = async (req, res, next) => {
  try {
    ...
    console.error(error);
    next(ApiError.internal("Ошибка при получении комментарией"));
  }
};
